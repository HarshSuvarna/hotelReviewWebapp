...
SERVICE_ACCOUNT_FILE = r'C:\Users\adity\Desktop\IT project\firebase-credentials\credentials(Do not delete).json'
...
def login(request):
    ...
    return render(request, 'login.html', {'error_message': request.GET.get('error_message', '')})
...
def signup(request):
    if request.method == 'POST':
        ...
        if len(password) < 6:
            return render(request, "signup.html", {"error": "Password should be at least 6 characters"})

        try:
            # Get the current highest user ID from Firestore
            highest_user = db.collection('users').order_by('uid', direction=firestore.Query.DESCENDING).limit(1).get()
            highest_uid = 0
